      ...
      "rules" : { for rule_idx, rule in plan["rules"] : coalesce(rule["name"], rule_idx) =>
        {
          for k, v in {
            ...
            "start_backup_window_minutes" : rule["start_backup_window_minutes"] == null ? null : { "@@assign" : rule["start_backup_window_minutes"] },
            "complete_backup_window_minutes" : rule["complete_backup_window_minutes"] == null ? null : { "@@assign" : rule["complete_backup_window_minutes"] }
          }
          : k => v if v != null
        }
      }
