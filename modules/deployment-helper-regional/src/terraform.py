...
def apply(
    tf_binary, var_file=None, destroy=False, auto_approve=True, additional_args=None
):
    """
    ...
    Args:
        ...
        destroy (bool, optional): Whether to run in destroy mode. Default: False
        auto_approve (bool, optional): Whether to auto-approve changes. Default: True
    ...
    LOGGER.info(
        f"Running terraform apply (action={action}) with command: {' '.join(cmd)}"
    )
    try:
        result = subprocess.run(
            cmd,
